...
for the Fibroblast and Non.myelinated.Shwan cells
...
Select Fibroblasts and Non.mielinated.Schw interactions. Retain gene_a and gene_b information

```{r}
n_col <- colnames(data)
idx_gene <- which(n_col %in% c("gene_a", "gene_b"))
idx_cells <- which(str_detect(string = n_col, pattern = "Fibroblast") &
                     str_detect(string = n_col, pattern = "Non.myelinated.Schw"))
...
```{r}
data <- data %>% select(!(Fibroblast_6.Non.myelinated.Schw: Fibroblast_7.Non.myelinated.Schw))
```
...
Filter rows not significant in the seven Fibroblast groups
...
arr <- data.frame(c(rep("MDK", 3), rep("PTN", 3)), c("LRP1", "PTPRZ1", "ALK", "PTPRZ1", "PTPRS", "ALK"), rep(1, 6))
arr2 <- data.frame(c(rep("MDK", 3), rep("PTN", 3)), c("LRP1", "PTPRZ1", "ALK", "PTPRZ1", "PTPRS", "ALK"), 1:6)
arr2 <- data.frame(c(rep("MDK", 3), rep("PTN", 3)), c("LRP1", "PTPRZ1", "ALK", "PTPRZ1", "PTPRS", "ALK"), rep(2, 6))
arrows <- data.frame(c(rep("MDK", 3), rep("PTN", 3)), c("LRP1", "PTPRZ1", "ALK", "PTPRZ1", "PTPRS", "ALK"), rep("black", 6))
  
pdf(file = here("chord_plot_Shwan_220825.pdf"), width = 7, height = 7, bg = "white")
...
```{r}
max(strwidth(unlist(dimnames(adj2)))+1)
```
