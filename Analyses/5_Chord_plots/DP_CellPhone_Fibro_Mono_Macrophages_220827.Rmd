...
for the Fibroblast and Monocytes.Macrophages cells
...
Select Fibroblasts and Monocytes.Macrophages interactions. Retain gene_a and gene_b information

```{r}
n_col <- colnames(data)
idx_gene <- which(n_col %in% c("gene_a", "gene_b"))
idx_cells <- which(str_detect(string = n_col, pattern = "Fibroblast") &
                     str_detect(string = n_col, pattern = "Monocytes.Macrophages"))
...
colnames(data)
```
Then, we should remove the extra columns
...
```{r}
data <- data %>% select(!(Fibroblast_6.Monocytes.Macrophages: Fibroblast_7.Monocytes.Macrophages))
```
...
arrows <- data.frame(c(rep("MDK", 2)), c("LRP1",  "SORL1"), rep("black", 2))

pdf(file = here("chord_plot_Monocytes_Macrophages_220827.pdf"), width = 7, height = 7, bg = "white")
