...
for the Fibroblast and MSC cells
...
Select Fibroblasts and MSC interactions. Retain gene_a and gene_b information

```{r}
n_col <- colnames(data)
idx_gene <- which(n_col %in% c("gene_a", "gene_b"))
idx_cells <- which(str_detect(string = n_col, pattern = "Fibroblast") &
                     str_detect(string = n_col, pattern = "MSC"))
...
arrows <- data.frame(c(rep("MDK", 1), rep("PTN", 1)), c("LRP1","PTPRS"), rep("black", 2))
  
pdf(file = here("chord_plot_MSC_220831.pdf"), width = 7, height = 7, bg = "white")
