...
for the Fibroblast and Odontoblast cells
...
Select Fibroblasts and Odontoblast interactions. Retain gene_a and gene_b information

```{r}
n_col <- colnames(data)
idx_gene <- which(n_col %in% c("gene_a", "gene_b"))
idx_cells <- which(str_detect(string = n_col, pattern = "Fibroblast") &
                     str_detect(string = n_col, pattern = "Odontoblast"))
...
pdf(file = here("chord_plot_Odontoblast_220827.pdf"), width = 7, height = 7, bg = "white")
